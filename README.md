...
## Установка руссификатора для Gunsaw Genetics (ПОКА НЕ РАБОТАЕТ, СОБСТВЕННО, ПОЭТОМУ НЕТ И ФАЙЛА)
- (разраб мода ещё не обновил мод, и пока это не работает, но оно будет работать примерно так)
- Скачай JSON-файл (RU-exp-gen.json) (**ОН В РАЗРАБОТКЕ**)
...
- Закинь туда файл
- Затем через конфиг (который в \Casualties Unknown Demo\BepInEx\config) замени строку Language = en на Language = ru-exp-gen (НЕ ДОПИСЫВАЙ .json!)
