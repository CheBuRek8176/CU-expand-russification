...
## ВНИМАНИЕ! Обновлений не будет до Progress Update (8.0)! (Кроме критически необходимых)

Автор сейчас занят переносом русификатора на 8 версию игры, а также разработкой русификатора для Casualty Vitals и Gunsaw Genetics.(последнее - пока заброшено, пока xi0n и thepristineant не доделают английские диалоги)

## Инфа по поводу новой версии

В source-файле находится почти доделанная версия русификатора, и она может быть маленько нестабильной. Рекомендуется скачивать через вкладку Releases (на ПК - справа, на мобильной версии - снизу).
...
**РУСИФИКАТОР РАБОТАЕТ ТОЛЬКО НА ВЕРСИИ ИГРЫ 7.0.1**
...
## Установка
**Внимание!** Если шаг по установке требует переименовать файл (например, в `RU-expand.json`), и у тебя изначально не отображается расширение (.json) (по умолчанию в Windows), указывать расширение не нужно (должно выйти типа `RU-expand`)! Если отображается, то расширение указывать уже нужно (`RU-expand.json`)

![скрин для тех, кто не понял](scrsh/ustanovka.png)


уупс, я опечатался и написал RU-expand/json.json))) я пытался написать RU-expand.json.json, типа что оно будет так числиться))) как-нибудь, когда время будет, пофикшу
### Первая установка базового русификатора
...
- В игре в гл. меню тыкни на кнопку с mp3-плеером (credits), затем кнопку с папкой (open locale folder). У тебя откроется папка с локалками (если нет - попробуй альт-табнуться). Туда закинь файл.

![тоже скрин для тех, кто не понял](scrsh/ustanovka2.png)


- Перезайди в игру, затем зайди в настройки (крайняя левая кнопка с синей капсулой) и настройки языка, после чего выбери "Русский+"
- Всё!
...
- Затем через конфиг (который в `\Casualties Unknown Demo\BepInEx\config`, открывается блокнотом) замени строку `Language = en` на `Language = ru` (НЕ ДОПИСЫВАЙ .json!)

### Установка русификатора для Casualty Vitals (В РАЗРАБОТКЕ)
- Скачай JSON-файл `cv_RU-expand` (ОН ТОЖЕ В РАЗРАБОТКЕ) и переименуй его в `cv_ru_RU.json` (мод тоже не дружит с другими названиями) и закинь его по пути `папка игры\BepInEx\plugins\Assets\CasualtyVitals\`. ОС скажет, что файл с таким именем уже есть. Замени его
- В игре в настройках мода (под кнопкой выхода) переключи язык на ru_RU
## Особенности
 ...
 - Скоро поддержка Gunsaw Genetics и Casualty Vitals!
...
- [Gunsaw Genetics](https://www.nexusmods.com/scavprototype/mods/324) - (Запланировано) Мод на персонажей из Gunsaw, пока находится в разработке, перевод временно заморожен (примерно до сентября-октября, может вообще до 27-го)
- [Casualty Vitals](https://www.nexusmods.com/scavprototype/mods/388) - (В разработке) Мод, который переделывает ЭКГ и добавляет пару новых механик, завязанных на сердечно-сосудистой системе
...
- StragAz - перевёл (пока переводит) большую часть реплик Милки и Дюны, фиксит некоторые грамматические нюансы
### Со-авторы
...
- Nexeus Calmless - я очень долго мучался с ним (до сих пор), пытаясь разобраться с какой проблемой он столкнулся. Добавил сюда чисто по приколу и за актив на посте
...
- 26.04.26 - первый релиз версии v6.0.1 (v0.1 по новой нумерации) и тест от Day1xt
- 10.05.26 - обнова v6.1.1 (v0.2)
- 04.06.26 - обнова v7.0.1.0 (v0.3) - прямо в день релиза версии v7.0.1 игры!
- 07.06.26 - 500+ просмотров за 3 дня - СПАСИБО ОГРОМНОЕ!
- 08.06.26 - обнова v7.0.1.1 (v0.3.1)
- 14.06.26 - 1000+ просмотров!
- 15.06.26 - обнова v7.0.1.2 (v0.3.2)
- 16.06.26 - хотфикс v7.0.1.2.1 (v0.3.2.1)
- 21.06.26 - 1500+ просмотров!
- 28.06.26 - 2500+ просмотров!
- 02.07.26 - обнова v7.0.1.3 (v0.3.3)
- 05.07.26 - хотфикс v7.0.1.3.1 (v0.3.3.1) и 3500+ просмотров!
...
- 18.09.26 - новая версия русификатора (v0.4.0-pre13) полноценно заработала. За 5 месяцев суммарно было собрано около 7000 просмотров
